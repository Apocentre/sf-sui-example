use eyre::Result;
use protos::pb::{
  sui::{CheckpointData, Transaction},
...
fn get_tx_sender(transaction: &[Transaction], digest: &str) -> String {
...
pub fn create_sui_coin(checkpoint_data: CheckpointData) -> Result<Vec<Coin>> {
  ...
  for coin_created in coins_created {
    info!("{:?}", coin_created);
    let creator = get_tx_sender(&checkpoint_data.transactions, &coin_created.previous_transaction.unwrap());
  }
